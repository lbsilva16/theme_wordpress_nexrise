...
<head>
  ...
  <style>
    /* CSS Crítico do Logo - Previne FOUI (Flash of Unstyled Image) */
    /* Este CSS é carregado antes do wp_head() para garantir que as regras sejam aplicadas imediatamente */
    .logo img,
    ...
    <div class="logo">
        ...
        if ( has_custom_logo() ) {
            ...
            if ( $logo ) {
                ...
                // Calcular dimensões proporcionais para max-height de 64px
                // Isso previne layout shift e FOUI
                $max_height = 64;
                if ( $logo_height > 0 ) {
                    $aspect_ratio = $logo_height / $logo_width;
                    $calculated_width = round( $max_height / $aspect_ratio );
                } else {
                    $calculated_width = 200; // fallback
                }
                
                echo '<a href="' . esc_url( home_url( '/' ) ) . '" class="custom-logo-link" rel="home">';
                echo '<img src="' . esc_url( $logo_url ) . '" alt="' . esc_attr( $alt_text ) . '" width="' . esc_attr( $calculated_width ) . '" height="' . esc_attr( $max_height ) . '" style="max-height: 64px; width: auto; height: auto;">';
                echo '</a>';
            } else {
                // Fallback caso não consiga obter a imagem
                the_custom_logo();
            }
        } else {
