...
$packages = [
	[
		...
		'price'    => esc_html__( 'Starting at $590 USD (or local equivalent)', 'nw-avada-like' ),
	...
	[
		...
		'price'      => esc_html__( 'Starting at $1,300 USD (or local equivalent)', 'nw-avada-like' ),
	...
	[
		...
		'price'    => esc_html__( 'Starting at $2,700 USD (or local equivalent)', 'nw-avada-like' ),
...
<section id="packages" class="nx-section packages-modern" data-packages>
	<div class="nx-container">
		<header class="packages-modern__header">
			...
			<p class="packages-modern__subtitle"><?php esc_html_e( 'PAY ONLY WHEN YOUR WEBSITE IS LIVE', 'nw-avada-like' ); ?></p>
		</header>
			...
			<?php foreach ( $packages as $index => $package ) : ?>
				...
				<article class="<?php echo esc_attr( implode( ' ', $card_classes ) ); ?>" data-card-index="<?php echo esc_attr( $index ); ?>">
					...
					<header class="packages-modern__card-header">
						...
						<div class="packages-modern__card-price-wrapper">
							<?php
							// Extrai o valor principal e a nota do preço
							$price_text = $package['price'];
							// Procura por padrão: "Starting at $XXX USD (or local equivalent)"
							if ( preg_match( '/Starting at (\$[\d,]+ USD)\s*(\(or local equivalent\))/i', $price_text, $matches ) ) {
								$price_amount = $matches[1]; // Ex: "$590 USD"
								$price_note = $matches[2];   // Ex: "(or local equivalent)"
								?>
								<div class="packages-modern__card-price">
									<span class="packages-modern__price-label">Starting at</span>
									<span class="packages-modern__price-amount"><?php echo esc_html( $price_amount ); ?></span>
								</div>
								<p class="packages-modern__price-note"><?php echo esc_html( $price_note ); ?></p>
								<?php
							} else {
								// Fallback: se o padrão não corresponder, exibe o texto completo
								?>
								<p class="packages-modern__card-price"><?php echo esc_html( $price_text ); ?></p>
								<?php
							}
							?>
						</div>
